...
def Cart(request):
    ...
    return render(request,'Cart.html',{'user':item,'mem':mem})
...
def remove_from_cart(request,pk):
    item=cart.objects.get(product_id=pk)
    item.delete()
...
def category_details(request,pk):
     ...
     current_user=request.user.id
     mem=member.objects.filter(user_id=current_user)
     return render(request,'category_details.html',{'mem':mem,'cat':cat,"prod":prod})
